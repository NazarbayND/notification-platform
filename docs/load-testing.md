...
Copy values from `tests/load/thresholds.env.example` into the environment to make acceptance thresholds explicit. k6 summaries, backlog drain time, and Prometheus platform metrics are written under `build/load-results/`.
...
Full orchestrator/worker/projection throughput measurements have not been run for the Phase 4–10 implementation and are intentionally deferred. No final-architecture benchmark result is available. Overload success means prompt explicit 429/503 responses, bounded memory/concurrency, retained Kafka data, and backlog recovery—not zero rejection.
