...
class EmailKafkaDeliveryConsumer {
    ...
    private final EmailWorkerServiceApplication.EmailDeliveryProcessor processor;private final KafkaWorkerCoordinator coordinator;
    EmailKafkaDeliveryConsumer(ObjectMapper mapper,JdbcTemplate jdbc,EmailWorkerServiceApplication.EmailDeliveryProcessor processor,
            KafkaTemplate<Object,Object> kafka,MeterRegistry meters,@Value("${worker.provider.max-concurrency:32}")int concurrency,
            @Value("${worker.provider.rate-per-second:100}")int rate){this.mapper=mapper;this.jdbc=jdbc;this.processor=processor;
    ...
    void consume(ConsumerRecord<String,String> record,Acknowledgment ack){String json=record.value();
        ...
        try{var job=new EmailWorkerServiceApplication.DeliveryJob(UUID.fromString(d.eventId()),UUID.fromString(d.notificationId()),
                UUID.fromString(d.deliveryId()),d.recipientAddress(),d.subject(),d.body());
            coordinator.handle(d,record,ack,()->processor.process(job),()->lookup(job.eventId()));}
